...
$tool = Join-Path $PSScriptRoot 'tools\SoundVolumeView\SoundVolumeView.exe'
if (-not (Test-Path -LiteralPath $tool)) {
    throw "SoundVolumeView.exe not found. Run setup.ps1 first."
}
...
function Read-CurrentDeviceFormat {
    ...
    if ($DeviceId) {
        ...
        if (-not $device) {
            $device = $devices | Where-Object { $_.Default -eq 'Render' } | Select-Object -First 1
        }
    }
